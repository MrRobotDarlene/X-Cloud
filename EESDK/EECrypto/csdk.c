...
static void create_bucket_request_worker(uv_work_t *work)
{
    ...
    if (req->response != NULL) {
        req->bucket = malloc(sizeof(bucket_meta_t));
...
static void get_buckets_request_worker(uv_work_t *work)
{
    ...
    if (num_buckets > 0) {
        req->buckets = malloc(sizeof(bucket_meta_t) * num_buckets);
    ...
    for (int i = 0; i < num_buckets; i++) {
        ...
        bucket_meta_t *bucket = &req->buckets[i];
        ...
        } else if (error_status == META_DECRYPTION_ERROR){
            bucket->decrypted = false;
...
static void list_files_request_worker(uv_work_t *work)
{
    ...
    if (num_files > 0) {
        req->files = malloc(sizeof(file_meta_t) * num_files);
...
static void get_file_info_request_worker(uv_work_t *work)
{
    ...
    req->file = malloc(sizeof(file_meta_t));
...
static void log_formatter(log_options_t *options,
                          void *handle,
                          int level,
                          const char *format,
                          va_list args)
...
static void log_formatter_debug(log_options_t *options, void *handle,
                                const char *format, ...)
{
    va_list args;
...
static void log_formatter_info(log_options_t *options, void *handle,
                               const char *format, ...)
{
    va_list args;
    va_start(args, format);
...
static void log_formatter_warn(log_options_t *options, void *handle,
                               const char *format, ...)
{
    va_list args;
    va_start(args, format);
...
static void log_formatter_error(log_options_t *options, void *handle,
                                const char *format, ...)
{
    va_list args;
    va_start(args, format);
...
API struct s_env *init_env(bridge_options_t *options,
                                 encrypt_options_t *encrypt_options,
                                 http_options_t *http_options,
                                 log_options_t *log_options)
{
    ...
    s_env_t *env = malloc(sizeof(s_env_t));
    if (!env) {
        return NULL;
    }
    ...
    bridge_options_t *bo = malloc(sizeof(bridge_options_t));
    if (!bo) {
        return NULL;
    }
    ...
    encrypt_options_t *eo = malloc(sizeof(encrypt_options_t));
    if (!eo) {
        return NULL;
    }
    ...
    } else if (getenv("TEMP_ENV") &&
               stat(getenv("TEMP_ENV"), &sb) == 0 &&
               S_ISDIR(sb.st_mode)) {
        env->tmp_path = strdup(getenv("TEMP_ENV"));
#ifdef _WIN32
    } else if (getenv("TEMP") &&
    ...
    http_options_t *ho = malloc(sizeof(http_options_t));
    if (!ho) {
        return NULL;
    }
    ...
    log_levels_t *log = malloc(sizeof(log_levels_t));
    if (!log) {
        return NULL;
    }

    log->debug = (logger_format_fn)noop;
    log->info = (logger_format_fn)noop;
    log->warn = (logger_format_fn)noop;
    log->error = (logger_format_fn)noop;
...
API int encrypt_auth(const char *passphrase,
                             ...
                             const char *mnemonic)
{
    ...
    char *buffer = NULL;
    if (encrypt_auth(passphrase, bridge_user,
                            ...
                            char **mnemonic)
{
    ...
    int status = decrypt_auth(buffer, passphrase, bridge_user,
...
API char *s_strerror(int error_code)
{
    switch(error_code) {
        ...
        case UNMAPPING_ERROR:
            return "Memory mapped file unmap error";
        case QUEUE_ERROR:
            return "Queue error";
        case HEX_DECODE_ERROR:
...
API int bridge_get_info(s_env_t *env, void *handle, uv_after_work_cb cb)
{
...
API int bridge_get_buckets(s_env_t *env, void *handle, uv_after_work_cb cb)
{
    uv_work_t *work = uv_work_new();
    if (!work) {
        return MEMORY_ERROR;
