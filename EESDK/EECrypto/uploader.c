...
static void print_shard_info(upload_state_t *state, int index) {
    shard_tracker_t *shard = &state->shard[index];
    shard_meta_t *shard_meta = state->shard[index].meta;
    host_pointer_t *p = state->shard[index].pointer;
    ...
    for (int i = 0; i < SHARD_CHALLENGES; i++ ) {
...
static exchange_report_t *exchange_report_new()
{
    exchange_report_t *report = malloc(sizeof(exchange_report_t));
    if (!report) {
        return NULL;
    }
    ...
    report->send_status = REPORT_NOT_PREPARED; // not sent
    report->start = 0;
    report->end = 0;
    report->code = 0;
    report->send_count = 0;
...
{
    host_pointer_t *pointer = calloc(sizeof(host_pointer_t), sizeof(char));
    if (!pointer) {
        return NULL;
    }
    pointer->token = NULL;
...
static encryption_ctx_t *prepare_encryption_ctx(uint8_t *ctr, uint8_t *pass)
{
    encryption_ctx_t *ctx = calloc(sizeof(encryption_ctx_t), sizeof(char));
    if (!ctx) {
        return NULL;
    }
...
static void cleanup_state(upload_state_t *state)
{
    ...
    if (state->shard) {
        for (unsigned int i = 0; i < state->total_shards; i++ ) {
...
static void after_create_bucket_entry(uv_work_t *work, int status)
{
    ...
    upload_state_t *state = req->upload_state;
    ...
    if (req->status_code == 200 || req->status_code == 201) {
        ...
        state->info = malloc(sizeof(file_meta_t));
        ...
        state->error_status = BRIDGE_REQUEST_ERROR;
    }
...
static int prepare_bucket_entry_hmac(upload_state_t *state)
{
...
static void after_push_shard(uv_work_t *work, int status)
{
    ...
    if (status == UV_ECANCELED) {
        ...
        shard->report->send_status = REPORT_NOT_PREPARED;
        goto clean_variables;
    }
    ...
    if (!req->error_status &&
        ...
        shard->report->code = REPORT_SUCCESS;
        shard->report->message = REPORT_SHARD_UPLOADED;
        shard->report->send_status = REPORT_AWAITING_SEND;

    } else if (!state->canceled){

        // Update the exchange report with failure
        shard->report->code = REPORT_FAILURE;
        shard->report->message = REPORT_UPLOAD_ERROR;
        shard->report->send_status = REPORT_AWAITING_SEND;
        ...
        if (shard->push_shard_request_count == 6) {
            ...
            state->error_status = HOST_REQUEST_ERROR;
        } else {
            req->log->warn(state->env->log_options, state->handle,
...
static void push_shard(uv_work_t *work)
{
    ...
    encryption_ctx_t *encryption_ctx = NULL;
    if (!state->rs) {
...
static void progress_put_shard(uv_async_t* async)
{
    ...
    upload_state_t *state = progress->state;
...
static void queue_push_shard(upload_state_t *state, int index)
{
    ...
    if (state->shard[index].report->farmer_id != NULL) {
        ...
        state->shard[index].report = exchange_report_new();
    }
    ...
    exchange_report_t *report = state->shard[index].report;
...
static void after_push_frame(uv_work_t *work, int status)
{
    ...
    upload_state_t *state = req->upload_state;
    host_pointer_t *pointer = req->farmer_pointer;
    ...
    if (req->status_code == 429 || req->status_code == 420) {
        ...
        host_pointer_t *p = state->shard[req->shard_meta_index].pointer;
        ...
        state->log->info(
            ...
            p->farmer_node_id
               ...
               STATUS_MAX_PUSH_FRAME_COUNT) {
        state->error_status = BRIDGE_OFFER_ERROR;
    } else {
        state->shard[req->shard_meta_index].progress = AWAITING_PUSH_FRAME;
    }
...
static void push_frame(uv_work_t *work)
{
    ...
    if (!json_object_object_get_ex(response, "token", &obj_token)) {
        req->error_status = BRIDGE_JSON_ERROR;
        goto clean_variables;
    }
...
static void prepare_frame(uv_work_t *work)
{
    ...
    struct sha256_ctx first_sha256_for_leaf[SHARD_CHALLENGES];
    for (int i = 0; i < SHARD_CHALLENGES; i++ ) {
    ...
    do {
        ...
        if (read_bytes == -1) {
            req->log->warn(state->env->log_options, state->handle,
                           ...
                           errno);
            req->error_status = FILE_READ_ERROR;
            goto clean_variables;
        }
...
static void after_create_encrypted_file(uv_work_t *work, int status)
{
    ...
    if (req->error_status != 0 || state->file_size != encrypted_file_size) {
        ...
        if (state->create_encrypted_file_count == 6) {
            state->error_status = FILE_ENCRYPTION_ERROR;
        }
    } else {
        state->log->info(state->env->log_options, state->handle,
        ...
        if (!state->encrypted_file) {
            state->error_status = FILE_READ_ERROR;
        }
    }
...
static void create_encrypted_file(uv_work_t *work)
{
    ...
    // Initialize the encryption context
    encryption_ctx_t *encryption_ctx = prepare_encryption_ctx(state->encryption_ctr,
                                                                    state->encryption_key);
    if (!encryption_ctx) {
        state->error_status = MEMORY_ERROR;
        goto clean_variables;
...
static void after_request_frame_id(uv_work_t *work, int status)
{
    ...
    if (req->status_code == 429 || req->status_code == 420) {
        ...
        state->error_status = BRIDGE_FRAME_ERROR;
    }
...
static void after_create_parity_shards(uv_work_t *work, int status)
{
    ...
    if (req->error_status != 0) {
        ...
        state->error_status = FILE_PARITY_ERROR;
    } else {
        state->log->info(state->env->log_options, state->handle,
...
static void create_parity_shards(uv_work_t *work)
{
    ...
    for (unsigned int i = 0; i < state->total_parity_shards; i++) {
...
static void queue_create_parity_shards(upload_state_t *state)
{
    uv_work_t *work = uv_work_new();
    if (!work) {
        state->error_status = MEMORY_ERROR;
...
static void after_send_exchange_report(uv_work_t *work, int status)
{
    ...
    if (status == UV_ECANCELED) {
        ...
        req->report->send_status = REPORT_AWAITING_SEND;
    ...
    if (req->status_code == 201) {
        ...
        req->report->send_status = REPORT_NOT_PREPARED; // report has been sent
    } else if (req->report->send_count == 6) {
        req->report->send_status = REPORT_NOT_PREPARED; // report failed retries
    } else {
        req->report->send_status = REPORT_AWAITING_SEND; // reset report back to unsent
    }
...
{
    shard_send_report_t *req = work->data;
    upload_state_t *state = req->state;
...
static void queue_send_exchange_report(upload_state_t *state, int index)
{
    ...
    req->report->send_status = REPORT_SENDING;
    req->state = state;
...
static void verify_bucket_id_callback(uv_work_t *work_req, int status)
{
    ...
    upload_state_t *state = req->handle;
    ...
    if (req->status_code == 200) {
        ...
        state->error_status = BRIDGE_BUCKET_NOTFOUND_ERROR;
    } else {
        state->log->error(state->env->log_options, state->handle,
...
{
    state->pending_work_count += 1;
    bridge_get_bucket(state->env, state->bucket_id, state, verify_bucket_id_callback);
}
...
static void verify_file_name_callback(uv_work_t *work_req, int status)
{
    ...
    if (req->status_code == 404) {
        ...
        state->error_status = BRIDGE_BUCKET_FILE_EXISTS;
    } else {
        state->log->error(state->env->log_options, state->handle,
                          "Request failed with status code: %i", req->status_code);
...
static void queue_verify_file_name(upload_state_t *state)
{
    state->pending_work_count += 1;
...
static void queue_push_frame_and_shard(upload_state_t *state)
{
    for (int index = 0; index < state->total_shards; index++) {

        if (state->shard[index].progress == AWAITING_PUSH_FRAME &&
            state->shard[index].report->send_status == REPORT_NOT_PREPARED &&
...
{
    log_levels_t *log = state->log;
    log_options_t *log_options = state->env->log_options;
    ...
    for (int index = 0; index < state->total_shards; index++ ) {
        if (state->shard[index].report->send_status == REPORT_AWAITING_SEND) {
...
{
    upload_state_t *state = work->data;
...
static void prepare_upload_state(uv_work_t *work)
{
    ...
    if(_fstati64(fileno(state->original_file), &st) != 0) {
        state->error_status = FILE_INTEGRITY_ERROR;
        return;
    }
#else
    struct stat st;
    ...
    if (!state->shard_size || state->shard_size == 0) {
        state->error_status = FILE_SIZE_ERROR;
        return;
    }
    ...
    for (int i = 0; i < state->total_shards; i++) {
        ...
        state->shard[i].report = exchange_report_new();
        if (!state->shard[i].report) {
            state->error_status = MEMORY_ERROR;
            return;
        }
    ...
    if (key_status) {
        switch (key_status) {
            case 2:
                state->error_status = HEX_DECODE_ERROR;
                break;
            default:
                state->error_status = MEMORY_ERROR;
        }
                            ...
                            finished_upload_cb finished_cb)
{
    ...
    upload_state_t *state = malloc(sizeof(upload_state_t));
    if (!state) {
        return NULL;
    }
