...
static void free_exchange_report(exchange_report_t *report)
{
...
static void free_download_state(download_state_t *state)
{
    for (int i = 0; i < state->total_pointers; i++) {
        s_pointer_t *pointer = &state->pointers[i];
...
static void request_pointers(uv_work_t *work)
{
    ...
    download_state_t *state = req->state;
...
static void set_pointer_from_json(download_state_t *state,
                                  s_pointer_t *p,
                                  struct json_object *json,
                                  bool is_replaced)
{
    if (!json_object_is_type(json, json_type_object)) {
        state->error_status = BRIDGE_JSON_ERROR;
        return;
    }
    ...
    p->report = malloc(
        sizeof(exchange_report_t));

    if (!p->report) {
        state->error_status = MEMORY_ERROR;
        return;
    }
    ...
    p->report->start = 0;
    p->report->end = 0;
    p->report->code = REPORT_FAILURE;
    p->report->message = REPORT_DOWNLOAD_ERROR;
...
static void append_pointers_to_state(download_state_t *state,
    ...
    if (length == 0) {
        ...
        if (state->total_pointers > 0) {
            state->pointers = realloc(state->pointers,
                                      total_pointers * sizeof(s_pointer_t));
        } else {
            state->pointers = malloc(length * sizeof(s_pointer_t) * 100);
        }
        if (!state->pointers) {
            state->error_status = MEMORY_ERROR;
            return;
        ...
        for (int i = 0; i < length; i++) {
            ...
            s_pointer_t *pointer = &state->pointers[j];
            if (pointer->parity) {
...
static void after_request_pointers(uv_work_t *work, int status)
{
    ...
    if (status != 0)  {
        ...
        state->error_status = BRIDGE_POINTER_ERROR;

    } else if (req->status_code == 429 || req->status_code == 420) {

        state->error_status = BRIDGE_RATE_ERROR;
        ...
        if (state->pointer_fail_count >= MAX_POINTER_TRIES) {
...
static void after_request_replace_pointer(uv_work_t *work, int status)
{
    ...
    if (status != 0) {
        ...
        state->error_status = BRIDGE_REPOINTER_ERROR;
...
static void queue_request_pointers(download_state_t *state)
{
    ...
    for ( unsigned int i = 0; i < state->total_pointers; i++) {

        s_pointer_t *pointer = &state->pointers[i];

        if (pointer->replace_count >= DEFAULT_MIRRORS) {
            state->log->warn(state->env->log_options,
                             state->handle,
        ...
        if (pointer->status == POINTER_ERROR_REPORTED) {
            ...
            if (status) {
                state->error_status = QUEUE_ERROR;
                return;
            }
...
static void request_shard(uv_work_t *work)
{
    ...
    if (error_status) {
        ...
        switch(status_code) {
            case 401:
            case 403:
                req->error_status = HOST_AUTH_ERROR;
                break;
            case 504:
                req->error_status = HOST_TIMEOUT_ERROR;
                break;
            default:
                req->error_status = HOST_REQUEST_ERROR;
        }
    } else {
        req->error_status = 0;
    }
...
static uint64_t calculate_data_filesize(download_state_t *state)
{
...
static void report_progress(download_state_t *state)
{
...
static void after_request_shard(uv_work_t *work, int status)
{
    ...
    s_pointer_t *pointer = &req->state->pointers[req->pointer_index];
    ...
    if (req->error_status) {
        ...
        req->state->log->warn(req->state->env->log_options,
                              ...
                              req->shard_hash,
                              s_strerror(req->error_status));

        pointer->status = POINTER_ERROR;

        switch(req->error_status) {
            case HOST_INTEGRITY_ERROR:
                pointer->report->code = REPORT_FAILURE;
                pointer->report->message = REPORT_FAILED_INTEGRITY;
            default:
                pointer->report->code = REPORT_FAILURE;
                pointer->report->message = REPORT_DOWNLOAD_ERROR;
        }
        ...
        pointer->report->code = REPORT_SUCCESS;
        pointer->report->message = REPORT_SHARD_DOWNLOADED;
...
static void progress_request_shard(uv_async_t* async)
{
    ...
    download_state_t *state = progress->state;
...
static void queue_request_shards(download_state_t *state)
{
    if (state->canceled) {
        return;
    }
...
static void after_send_exchange_report(uv_work_t *work, int status)
{
    ...
    if (req->report->send_count >= MAX_REPORT_TRIES ||
        req->status_code == 201) {
...
static void queue_send_exchange_reports(download_state_t *state)
{
    ...
    for (unsigned int i = 0; i < state->total_pointers; i++) {
        ...
        if (pointer->report->send_status < 1 &&
            pointer->report->send_count < MAX_REPORT_TRIES &&
...
static void determine_decryption_key_v1(download_state_t *state)
{
    uint8_t *index = NULL;
...
static void determine_decryption_key_v0(download_state_t *state)
{
...
static void determine_decryption_key(download_state_t *state)
{
...
static void after_request_info(uv_work_t *work, int status)
{
    ...
    if (status != 0) {
        req->state->error_status = BRIDGE_FILEINFO_ERROR;
        ...
        if (req->info->erasure) {
            if (strcmp(req->info->erasure, "reedsolomon") == 0) {
                ...
                req->state->error_status = FILE_UNSUPPORTED_ERASURE;
            }
        }
        ...
        switch(req->error_status) {
            case BRIDGE_REQUEST_ERROR:
            case BRIDGE_INTERNAL_ERROR:
        ...
        if (req->state->info_fail_count >= MAX_INFO_TRIES) {
...
static void request_info(uv_work_t *work)
{
    ...
    if (request_status) {
        ...
        req->error_status = BRIDGE_AUTH_ERROR;
    } else if (status_code == 404 || status_code == 400) {
        req->error_status = BRIDGE_FILE_NOTFOUND_ERROR;
    } else if (status_code == 500) {
        req->error_status = BRIDGE_INTERNAL_ERROR;
    } else {
        req->error_status = BRIDGE_REQUEST_ERROR;
    }
...
static void queue_request_info(download_state_t *state)
{
...
static int prepare_file_hmac(download_state_t *state)
{
...
static bool has_missing_shard(download_state_t *state)
{
    bool missing = false;
    for (int i = 0; i < state->total_pointers; i++) {
        s_pointer_t *pointer = &state->pointers[i];
...
static bool can_recover_shards(download_state_t *state)
{
    if (state->pointers_completed) {
...
static void after_recover_shards(uv_work_t *work, int status)
{
    ...
    if (status != 0) {
        req->state->error_status = QUEUE_ERROR;
    } else if (req->error_status) {
        req->state->error_status = req->error_status;
    } else {
...
static void recover_shards(uv_work_t *work)
{
    ...
    if (prefile == INVALID_HANDLE_VALUE) {
        req->error_status = FILE_RESIZE_ERROR;
        return;
    }
    ...
    if (error) {
        req->error_status = MAPPING_ERROR;
        goto finish;
    }
    ...
    for (unsigned int i = 0; i < req->data_shards; i++) {
        data_blocks[i] = data_map + i * req->shard_size;
    }

    for (unsigned int i = 0; i < req->parity_shards; i++) {
    ...
    if (error) {
        req->error_status = FILE_RECOVER_ERROR;
        goto finish;
    }
...
finish:
    if (data_map) {
        ...
        if (error) {
            req->error_status = UNMAPPING_ERROR;
        }
    }
...
static void queue_recover_shards(download_state_t *state)
{
...
static void queue_next_work(download_state_t *state)
{
    // report any errors
    ...
    if (state->pointers_completed &&
        ...
        if (!state->finished && state->pending_work_count == 0) {
            ...
            if (prepare_file_hmac(state)) {
                state->error_status = FILE_GENERATE_HMAC_ERROR;
            }

            if (state->info && state->info->hmac) {
                if (0 != strcmp(state->info->hmac, state->hmac)) {
                    state->error_status = FILE_DECRYPTION_ERROR;
                }
            } else {
                state->log->warn(state->env->log_options,
                                 state->handle,
    ...
    if (state->info) {
        ...
        if (state->rs) {
            if (can_recover_shards(state)) {
                ...
                state->error_status = FILE_SHARD_MISSING_ERROR;
                queue_next_work(state);
                return;
            }
        } else {
...
API int bridge_resolve_file_cancel(download_state_t *state)
{
    ...
    state->canceled = true;
    state->error_status = TRANSFER_CANCELED;
...
{
    download_state_t *state = malloc(sizeof(download_state_t));
    if (!state) {
        return NULL;
    }
    ...
    state->download_max_concurrency = DOWNLOAD_CONCURRENCY;
    ...
    state->pointer_fail_count = 0;
    state->requesting_pointers = false;
    state->error_status = TRANSFER_OK;
    state->writing = false;
    state->shard_size = 0;
