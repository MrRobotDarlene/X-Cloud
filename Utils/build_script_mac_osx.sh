...
cp ../../resources/xcloud.icns X\ Cloud.app/Contents/Resources
if [ $? -ne 0 ]; then
    echo "Failed to cp xcloud.icns to resources"
    exit $?
fi
...
hdiutil create -format UDZO -srcfolder package_folder -volname X\ Cloud "XCloud-macos-x64-$version_str.dmg"
if [ $? -ne 0 ]; then
